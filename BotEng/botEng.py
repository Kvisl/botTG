...
def create_cards(message):
    ...
    if word:
        ...
        other_words = [other.other_word for other in
                       session.query(Other).filter_by(id_translate=word.id).all()]
...
def add_word_other(message):
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        bot.set_state(message.from_user.id, MyStates.example_word,
                      message.chat.id)
...
def add_example(message):
    ...
    if word:
        ...
        bot.send_message(message.chat.id, f"Вы изучаете {total_words} слова.")
...
def delete_word_process(message):
    ...
    word_to_delete = session.query(Word).filter_by(user_id=user_id, target_word=target_word).first()
    if word_to_delete:
    ...
    if text == translate_word:
        ...
        if word:
            ...
            if word.attempts >= 5:
                ...
                answer = (f"Отлично!\U0001f600\n{data['target_word']} -> \U0001f1ec\U0001f1e7 {data['translate_word']} "
                          f"-> {data['example']}\nОсталось {5 - word.attempts} попыток до изучения слова '{data['translate_word']}'.")
            session.commit()
        else:
        ...
        answer = "\u274CДопущена ошибка! Попробуй ещё раз"
